...
def main(page: ft.Page):
    ...
    def check_word(event, page):
        nonlocal current_index, mistakes, correct_count
        ...
        total_attempts = correct_count + mistakes
        accuracy = (correct_count / total_attempts) * 100 if total_attempts > 0 else 100
        accuracy_label.value = f"Accuracy: {accuracy:.2f}%"
        accuracy_bar.value = (correct_count / total_attempts) if total_attempts > 0 else 1.0
